...
SessionListenerSessionMemberAddedFuncType = CallbackType(None, C.c_void_p, C.c_uint, C.c_char_p) # context sessionId uniqueName
...
SessionListenerSessionMemberRemovedFuncType = CallbackType(None, C.c_void_p, C.c_uint, C.c_char_p) # context sessionId uniqueName
...
class SessionListener(AllJoynObject):
    ...
    def  _OnSessionLostCallBack(context, sessionId, reason):  # const void* context, alljoyn_sessionid sessionId, alljoyn_sessionlostreason reason)
        self = AllJoynObject.unique_instances[unique_id]
